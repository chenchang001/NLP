...
#
# This is 2nd attempt
#  This is a threaded call for bert-as-my-service
#  Reference source https://github.com/ElementAI/multithreaded-estimators/blob/master/threaded_estimator/models.py
#  It could work
#  Two thread: main thread feed predict data(list type) in to queue by batch, sub-thread fetch the data from queue and send to predict via generator
#  it cannot work with TPU as generator is not TPU compatiable
#  the input_fn and convert_examples_to_features is reference to non-file-based version in bert-master
#  version c is a draft version and version d is finalized version
#######################################
...
flags.DEFINE_bool(
    "threaded", True, 
    "Whether to run estimator prediction in another thread.")
    
flags.DEFINE_bool(
    "verbose", False, 
    "Whether to print info for thread logic.")
...
class Predict_Threaded():
    def __init__(self):
        #Measure performance
        self.t1 = time.time()

        #Define two queues for data to predict and result
        self.input_queue = Queue(maxsize=FLAGS.predict_batch_size)
        self.output_queue = Queue(maxsize=FLAGS.predict_batch_size)
        tf.logging.info("Queue created.")
        
    def generate_from_queue(self):
            """ Generator which yields items from the input queue.
            """
            while True:
                feed = self.input_queue.get()
                if FLAGS.verbose:
                    tf.logging.info('Yielding from input queue')
                yield feed
                
    def queued_predict_input_fn(self, params):
        if FLAGS.verbose:
            tf.logging.info("QUEUED INPUT FUNCTION CALLED")
    ...
    def predict_from_queue(self):
        ...
        for prediction in estimator.predict(input_fn=self.queued_predict_input_fn):
            if FLAGS.verbose:
                tf.logging.info('Putting in output queue')
            self.output_queue.put(prediction)


    def predict_request(self,predict_examples,label_list,tokenizer):
            ...
            for feature in features:
                ...
                if count == FLAGS.predict_batch_size:
                    ...
                    for i in range(FLAGS.predict_batch_size):
                        r = self.output_queue.get()
                        if FLAGS.verbose:
                            tf.logging.info('Recieving from output queue')
    ...
    def prepare_bert_as_my_service_client(self):
        ...
        processor.tokenizer = tokenization.FullTokenizer(
            vocab_file=FLAGS.vocab_file, do_lower_case=FLAGS.do_lower_case)

        if FLAGS.do_predict:
            #warmup with file to inital model before ready to start client
            predict_examples = processor.get_test_examples(FLAGS.data_dir, "warmup-predict.tsv")
            warmup_result = self.predict_request(predict_examples,processor.label_list,processor.tokenizer)
            if FLAGS.verbose:
            ...
            tf.logging.info("Service is ready.")
            tf.logging.info("Start-up time: %s seconds " % (str(startup_time)))
...
def bert_client(threadservice):
    processor = threadservice.prepare_bert_as_my_service_client()

    try:
        while True:
            input_data = predict_data_input_fn()
            predict_examples = processor.get_test_examples_from_list(input_data)
            output_data = threadservice.predict_request(predict_examples,processor.label_list,processor.tokenizer)
        ...
        threadservice.stop_bert_as_my_service()
...
def main(_):
  ...
  tf.logging.info("-= Bert as my service =-\n")
  
  if not FLAGS.threaded:
    raise ValueError(
        "This service model version must have `threaded' be True ")
  
  threadservice = Predict_Threaded()
  
  if FLAGS.do_predict:
    if FLAGS.threaded:
      prediction_thread = Thread(target=threadservice.predict_from_queue, daemon=True)
      prediction_thread.start()
      tf.logging.info("Predict thread started.")

      bert_client(threadservice)
#BSMS end
